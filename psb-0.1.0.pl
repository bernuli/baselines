...
my $scriptVersion    = "0.1.0";
my $scriptStartTime  = time;


 
# 	psb (perlSimpleBenchmark) is a perl script that attemtps to measure a
# 	CPUs  multi tasking performance.  The basic premise is a separate process
#	is created, or forked, and that new process will run an empty foreach loop 
#	and measure the elapsed time to complete.
#
#	psb will first fork a single instance of an empty foreach loop (known as
#	the INNERLOOP).  The INNERLOOP looks like this:
#
#	for (1 .. 1000000000) {} # INNERLOOP
#
#	The OUTERLOOP is responsible for running the INNERLOOP and adds an
#	additional fork of the INNERLOOP on each pass.
#
...
# 
# 	The default number of passes is 8.
#
#	While psb is running, status and result output is sent to the terminal.  
#
#	Upon completion of all passes, CSV output is sent to a file as well as
#	the terminal.  The CSV output includes information about the hardware
#	and software that is running the psb script.
#
...
## Since perl versions vary in empt loop performance, lets try to stick
## with perl 5.18.
...
my @passTimes;				# Array of array.  For each pass store the number of forks and time to complete.
$passTimes[0]=["0","0"];

my @passReports;			# Array of array.  Store pass Reports
$passReports[0]=["0","0"];

my $ipsMaxPass = 1; 		# The pass # which ran the most ips.
...
my $usage = "\nUsage: $scriptName [-a -h -n -o -p]\n\n\t[-a] Anonymous. Omit machine name and SN from CSV output.\n\n\t[-h] Help. Display usage info.\n\n\t[-n] Notes. Text to add to Notes column of CSV output.  (Enclose text in double quotes if more than one word.)\n\n\t[-o] outDir. Specify directory for report output.\n\n\t[-p (1-$maxPasses)] Passes. Specify number of passes.\n\n\n\nEXAMPLES\n\n\t./$scriptName -p 3\n\n\t./$scriptName -p 3 -n \"clean install of macOS 11\"\n\n\t./$scriptName -n \"clean install of macOS 11\"\n\n\t./$scriptName -n clean\ install\ of\ macOS\ 11\n\n\t./$scriptName -o /Users/frank/theBase\n\n\t./$scriptName -o \"/Users/frank/The Base Lines\"\n\n";
...
if ($thisOS eq "win") {
	...
	($uname_or_ver) = commandResults("uname -mv");
}
...
## Script Version Info
push @commonRowData,  "$scriptName $scriptVersion benchmarkEngineVersion $benchmarkEngineVersion  Loop used:    for (0 .. $numberofInnerLoops ) {}";
...
## Absolute Times Row
push @absoluteTimesRow, "Absolute Time (Seconds)" ;
...
## Send a copy of reports to stdout.
print "\n\nCSV BEGIN\n";

print CSVheader();

print sendCSV(\@absoluteTimesRow);
...
## Set sendHeader Flag.  If this is a new file will want to send CSV header as first row.
my $sendHeader=0;
...
print FHO sendCSV(\@absoluteTimesRow);

## Write delta times row.
print FHO sendCSV(\@deltaTimesRow);

## Write ips row.
print FHO sendCSV(\@ipsRow);

## Write ips Delta row.
print FHO sendCSV(\@ipsDeltaRow);
...
sub perlSimpleBenchmarkEngine_0_3_0 {
	...
	return "Missing input parameters to $benchmarkEngineVersion" unless $_[0];
...
# 	$passTimes[0]=["0","0"];
 	...
 	for my $i ( 1 .. $#passTimes ) {
		...
		my $seconds    = $passTimes[$i][1];
		my $ipsCurrent = ($i * $numberofInnerLoops) / $passTimes[$i][1] ;
		my $ipsLast = 0;
		my $ipsDeltaPercent = 100;


		if ($i > 1) {

			$ipsLast         = ( ($i-1) * $numberofInnerLoops) / $passTimes[$i-1][1] ;
			$ipsDeltaPercent = ( $ipsCurrent - $ipsLast ) / $ipsLast * 100;

		}

		
		#$passReports[$i] = [ $forks, $seconds, $ipsCurrent, $ipsLast, $ipsDeltaPercent ];
		$passReports[$i] = [ "", $i, $seconds, $ipsCurrent, $ipsLast, $ipsDeltaPercent ];
	...
	for my $pi (1 .. $#passReports) {

		## Identify the pass that had the max ips.
		if ( $passReports[$pi][3] > $passReports[$ipsMaxPass][3] ) {  $ipsMaxPass=$pi;  }
		
		## Figure out column withs for pass totals report.

	} 
		
		
	print "Pass#    Processes    Seconds    ipsDelta%      ips/1000000\n";
	for my $pi (1 .. $#passReports) {

		my $seconds         = $passReports[$pi][2];
		my $ipsCurrent      = $passReports[$pi][3]; 
		my $ipsDeltaPercent = $passReports[$pi][5];
		my $ipsLast         = $passReports[$pi][4];
...
# 		printf("%-8s %-10s %-10s %-14s %s\n", $c1, $c2, $c3, $c4, $c5  );
	 	printf(
	 		...
	 		$passReports[$pi][1],
	 		sprintf("%.3f", $seconds),
	 		sprintf("%.3f",$ipsDeltaPercent),
	 		sprintf("%.3f", $ipsCurrent / 1000000)
	 		);

  		if ($pi == $ipsMaxPass) { print " <-- MAX ips" ; }
 		print "\n";
 		
		
 		#print "\t ipsCurrent: [$ipsCurrent] ipsLast: [$ipsLast]\n";
 	}
...
sub parseArgs {
	...
	for my $i (0 .. $#ARGV) { 
		...
		if ($ARGV[$i] eq "-h" || $ARGV[$i] eq "-H") { print $usage; exit 0;} 
		...
		if ($ARGV[$i] eq "-p") {
			unless ($ARGV[$i+1] =~ /^\d+$/) { print "ERROR: Number of passes must be a number.\n"; print $usage; exit; }
...
sub CSVheader {
	...
	push @header, "Run Date Time";            ## Now which is then.
	...
	push @header, "CPU Marketing String ( on macOS sysctl -a machdep.cpu.brand_string)"; ## from sysctl -a
	...
	push @header, "uname -mv or ver";        ## uname -a command
	push @header, "perl v";                 ## The value of variable $]
	push @header, "lsof count (user)";      ## lsof command
	push @header, "lsof count (root)";      ## * Not implemented at this time.  Users are welcome to get this on their own and add to CSV rather than run this script as root.
	push @header, "Benchmark Version";      ## The benchmarkEngineVersion.  That is the fork and loop code that is being timed.
	push @header, "Beginning Uptime";       ## uptime command
	push @header, "Ending Uptime";          ## uptime command
	push @header, "Notes";                  ## User supplied notes from command line options.
	push @header, "RSLTS TYPE";           ## A uniquie id given to this running of the loops.
...
sub sendCSV {
	...
	foreach (@$arrayRef) {
		...
		## CSV quoting and ;. 
		if ($_ =~ /\,/) {    ## If there is a comma in the field, then escape double quotes with double quote ( " becomes "" ), then surround field in double quotes.
			$_ =~ s/"/""/g;  ## Escape double quotes;
			$_ = "\"$_\"";  ## Enclose field in double quotes.
		} 
	...
	## Substitute the last comma with newline.
	$lineOfCsv =~ s/,$/\n/;
