...
# 	loop that iterates from 1 to 1000000000, known as the INNERLOOP.  The
# 	INNERLOOP looks like this:
# 
# 	foreach (1 .. 1000000000 ) {}    # INNERLOOP Empty foreach loop.
# 
...
# 	pass of the OUTERLOOP, another instance of the INNERLOOP is added.  The
# 	default number of passes of the OUTERLOOP is 8, so on the 8th pass there
# 	would be 8 separate processes running the INNERLOOP in parallel.
# 
# 	You can set the number of OUTERLOOP passes with the -p flag:
# 
# 	-p 9 would be 9 passes of the OUTERLOOP and would iterate from a single
# 	instance of the INNERLOOP up through 9 instances running in parallel.
# 
# 	While psb is running, status and result output is sent to the terminal.
# 
# 	Upon completion of all OUTERLOOP passes, CSV output is sent to the terminal
# 	as well as to a file. The CSV output includes information about the
# 	hardware and software that is running the psb script.
# 
# 	In addition to the CSV file, system info is gathered in separate files
# 	and saved to the machineInfo subdirectory.
...
my $scriptVersion    = "$scriptVersion[0].$scriptVersion[1].$scriptVersion[2]";
...
#  0.1.2  Added subroutines passDataUpdate and outputPassDataReport.   
#  0.1.3  renamed @header array to fieldNamesRow and reworked building of the array.  
...
my $sysctl_ref;				# Reference to array containing results of sysctl command.

my %machineDetails;			# Property and value of various machine specs.

my @cpuLookupTable;			# CPU data from ark.intel.com

my $outDir; 				# This is where we will write CSV file.

my $hostname = "hostname";	# Will become actual machine's hostname unless -a is specified.
...
for my $i (1 .. $numberOfPasses) {  ## OUTERLOOP
...
unless ($thisOS eq "win") {
	($endingUptime) = commandResults("uptime");
}
...
unless ($thisOS eq 'win') {
	...
	$sysctl_ref = sysctl_parse();

}


## If on windows get CPU cpu.marketing.string
if ($thisOS eq 'win') {
...
## Run Date
push @commonRowData,  timestamp_excel($scriptStartTime) . "";
...
if ($anonymous) {
	...
	push @commonRowData,  "Serial Number";
...
## ModelName
push @commonRowData,  getItemFromMachineDetails("Model Name");
...
## Processor Name
push @commonRowData,  getItemFromMachineDetails("Processor Name");
...
## Boot ROM Version
push @commonRowData,  getItemFromMachineDetails("Boot ROM Version");
...
## OS Details
push @commonRowData,  getItemFromMachineDetails("System Version");
...
push @commonRowData,  getItemFromMachineDetails("Max Turbo");
...
if ($thisOS eq "win") {
	...
	$uname_or_ver = winCommandResults("ver");
	
} else {

	($uname_or_ver) = commandResults("uname -a");
}
...
if ($thisOS eq "win") {
	...
	print "Getting lsof...\n";
 	@lsofData = commandResults ("lsof -n");
...
## Script Version Info
push @commonRowData,  "$scriptName $scriptVersion benchmarkEngineVersion $benchmarkEngineVersion  INNERLOOP: foreach (1 .. $numberofInnerLoops ) {}";
...
@fieldNamesRow = (	# Column names of data output.
	...
	"Serial Number",			## from system_profiler
	...
	#"RSLTS TYPE";				## A uniquie id given to this running of the loops.
	"Innerloops",				## Number of iterations of the INNERLOOP.

);

## Use a loop to add number pass columns using the defined maxPasses variable.
for my $i ( 1 .. $maxPasses ) {
	push @fieldNamesRow, "Pass #$i";
}




## Add common data to all rows.
foreach (@commonRowData) {

	push @absoluteTimesRow, $_ ;
# 	push @deltaTimesRow,    $_ ;
# 	push @ipsRow,           $_ ;
# 	push @ipsDeltaRow,      $_ ;
}
...
for my $i (1 .. $#passTimes) {
	...
	push @absoluteTimesRow, $passTimes[$i][1]  ;
...
## Delta Times Row
# push @deltaTimesRow, "Delta Times (seconds)" ;
# 
# push @deltaTimesRow, $absoluteTimesRow[25] ;
# 
# for my $i (2 .. $#passTimes) {
# 	
# 	my $loopTime         = $absoluteTimesRow[24+$i] ;
# 
# 	my $loopTimePrevious = $absoluteTimesRow[24+$i-1] ;
# 	
# 	my $loopTimeDelta    = ($loopTime-$loopTimePrevious) ; 
# 
# 
# 	push @deltaTimesRow, $loopTimeDelta ;
# 
# }
...
## ipsRow
# push @ipsRow, "IPS / 1000000"  ;
# 
# for my $i (1 .. $#passTimes) {
# 
# 	my $ips  =  ($numberofInnerLoops * $i) / $passTimes[$i][1] / 1000000 ;
# 
# 	push @ipsRow, $ips ;
# 
# }
...
## ips Delta Row
# push @ipsDeltaRow, "IPS Delta";
# 
# push @ipsDeltaRow, $ipsRow[25] ;
# 
# for my $i (2 .. $#passTimes) {
# 
# 	my $ips         = $ipsRow[24+$i] ;
# 
# 	my $ipsPrevious = $ipsRow[24+$i-1] ;
# 	
# 	my $ipsDelta    = ($ips-$ipsPrevious) ; 
# 
# 
# 	push @ipsDeltaRow, $ipsDelta ;
# 
# }
...
my $fieldNamesRowCSV = sendCSV(\@fieldNamesRow);  # String containing column names in CSV format.

# for my $i (0 .. $#fieldNamesRow-1) {
# 	$fieldNamesRowCSV .= "$fieldNamesRow[$i],";
# }
# 
# $fieldNamesRowCSV .= "$fieldNamesRow[$#fieldNamesRow]";
...
print "\r\n" . sendCSV(\@absoluteTimesRow);
# print sendCSV(\@deltaTimesRow);
# print sendCSV(\@ipsRow);
# print sendCSV(\@ipsDeltaRow);
...
## If a valid outDir is specified at runtime then use that, otherwise go with defaults.

if ( defined $outDir && -d $outDir && -w $outDir ) { 

	print "Will use outDir [$outDir]\n";
...
my $CSVCSVoutputFileName	= "$scriptName-$scriptVersion-$hostname.csv";
...
unless (-e $CSVoutputFile) { $sendHeader = 1; }
...
## Write delta times row.
# print FHO "\r\n" . sendCSV(\@deltaTimesRow);
# 
# ## Write ips row.
# print FHO "\r\n" . sendCSV(\@ipsRow);
# 
...
#### Send ouput of sysctl -a to a file

if ($sysctl_ref) {
	
	print "Getting a copy of sysctl -a to $outDir/machineInfo/sysctls\n";
	...
	unless (-e $sysctlFile) {
		...
		print $sysfo timestamp_excel($scriptStartTime) . chr(012);
...
unless ($thisOS eq "win") {
	...
	print "Getting a copy of ps aux to $outDir/machineInfo/ps\n";
	...
	unless (-e $psFile) {
		...
		print $psfo timestamp_excel($scriptStartTime) . chr(012);
...
unless ($thisOS eq "win") {
	...
	print "Getting a copy of lsof to $outDir/machineInfo/lsof\n";
	...
	my $lsof = commandResults("lsof -n");
	
	unless (-e $lsofFile) {
	
		open (my $lsoffo,">",$lsofFile) or warn "Could not open file for writing $!";

		print $lsoffo timestamp_excel($scriptStartTime) . chr(012);
...
my $passTotalsFile = "$outDir/$scriptName-$scriptVersion-$hostname-passTotals.txt";

print "Writing pass Totals to $passTotalsFile\n";

open (my $PTO, ">>", "$outDir/$scriptName-$scriptVersion-$hostname-passTotals.txt") or warn "Could not open file for writing. $!";

print $PTO "Run Date: " . timestamp_excel($scriptStartTime) . "\n";
print $PTO "Run UUID: $uuid[0]\n";
print $PTO "Pass Totals: (INNERLOOP $numberofInnerLoops)\n";
print $PTO outputPassDataReport();
...
print "\n\n";
print "Results were written to [$outDir]\n";
...
sub perlSimpleBenchmarkEngine_0_3_0 {
	...
	my $benchmarkEngineVersion = "0.3.0"; 

	return "benchmarkEngine $benchmarkEngineVersion" unless $_[0];

	my $forks = $_[0];

	unless ($forks =~ /^\d+$/ && $forks > 0) { my $caller = caller(); die "Bad input to $caller \n\$forks was [$forks]. Cannot continue. $!"; }
...
# 	push @passDataReport, "\nPass Totals: (INNERLOOP $numberofInnerLoops)\n" ;
	push @passDataReport, "\nPass Totals:\n" ;
	push @passDataReport, "INNERLOOP was $numberofInnerLoops\n" ;
	push @passDataReport, "ips = Iterations Per Second\n" ;

# 	push @passDataReport, ("                       Iterations\n");
# 	push @passDataReport, ("                       Per Second\n");
	push @passDataReport, ("Pass#  Prcss  Seconds  SecondsDlt  IPS/1000000  IPSdlt%  IPSdltTotal% \n");
	...
	foreach my $i (1 .. $#passData) {
		...
		my $passDataReportLine = sprintf (
			...
			"%-6s %-6s %-8s %-11s %-12s %-8s %s",
			$i, $passData[$i][1], sprintf("%.3f", $passData[$i][2]), sprintf("%.3f", $passData[$i][3]), sprintf("%.3f", $passData[$i][5]/1000000), sprintf("%.3f", $passData[$i][7]), sprintf("%.3f", $passData[$i][9]) 
...
sub getMaxTurbo {
	...
	foreach my $row (@cpuLookupTable){
		...
		foreach my $cpuBrandItem (@cpuBrandBreakdown) {
			#print "Matching [$Product_Name] and [$cpuBrandItem]\n";
...
sub parseArgs {
	...
	$legalArgs{'-o'} = 1; ## Specify outDir for report output.
	$legalArgs{'-p'} = 1; ## Number of passes to perform.
	$legalArgs{'-t'} = 1; ## Testing flag.  Use shorter innerloop.

	
	for my $i (0 .. $#ARGV) { 
		
		## User asked for help so print helpString and exit.
		if ($ARGV[$i] eq "-h" || $ARGV[$i] eq "-H") { print $helpString; exit 0;} 
		...
		## Extract digit after -p which is number of passes.
		if ($ARGV[$i] eq "-p") {
			unless ($ARGV[$i+1] =~ /^\d+$/) { print $helpString; print "ERROR: Number of passes must be a number.\n"; exit; }
			$numberOfPasses = $ARGV[$i+1];
			
			## Disallow too many passes.
			if ($numberOfPasses > $maxPasses) {
				print "Too many passes.  Max is $maxPasses. Try again.\n"; exit;
			}
			next;
		}


		## Extract notes from after -n flag.
		...
		if ( $ARGV[$i] eq "-o" ) {
			...
			unless (-d $outDir) {
				...
				print "ERROR!!! Invalid outDir.  Directory does not exist.\n\tCreate directory or specify a directory that already exists.\noutDir was [$outDir].\n";
				exit 1;
			...
			unless (-w $outDir) {
				...
				print "ERROR!!! Invalid outDir.  Directory not writable.\noutDir was [$outDir].\n";
				exit 1;
		...
		## Look for testing flag and set a lower number for INNERLOOP.
		if ( $ARGV[$i] eq "-t" ) {
			$testRun = 1;
			$numberofInnerLoops = 10000000;
			next;
...
# sub CSVheader {
	...
	# Gather column names into array to be used as the first line of CSV file.
# 	
# 	my @CSVheader;  ## CSV header.
# 	
# 	push @CSVheader, "Run Date Time";		## Now which is then.
# 	push @CSVheader, "Run UUID";			## A uniquie id given to this running of the loops.
# 	push @CSVheader, "Computer Name";		## from system_profiler
# 	push @CSVheader, "Serial Number";		## from system_profiler
# 	push @CSVheader, "Model Name";			## from system_profiler
# 	push @CSVheader, "Model Identifier";	## from system_profiler
# 	push @CSVheader, "Processor Name";		## from system_profiler
# 	push @CSVheader, "Processor Speed";	## from system_profiler
# 
# 	push @CSVheader, "Boot ROM Version/System Firmware Version";	## from system_profiler
# 
# 	push @CSVheader, "SMC Version (system)";	## from system_profiler
# 	push @CSVheader, "OS v";					## system_profiler
# 
# 	push @CSVheader, "CPU Marketing String ( on macOS sysctl -a machdep.cpu.brand_string)"; ## from sysctl -a
# 
# 	push @CSVheader, "Current Processor Speed";	## sysctl -a
# 	push @CSVheader, "Max Turbo (Intel Spec)";	## Look up in data gathered from ark.intel.com 
# 	push @CSVheader, "Cores";					## syctl -a machdep.cpu.core_count
# 	push @CSVheader, "Threads";					## from sysctl -a (machdep.cpu.thread_count)
# 	push @CSVheader, "uname -mv or ver";		## uname -a command
# 	push @CSVheader, "perl v";					## The value of variable $]
# 	push @CSVheader, "lsof count (user)";		## lsof command
# 	push @CSVheader, "lsof count (root)";		## * Not implemented at this time.  Users are welcome to get this on their own and add to CSV rather than run this script as root.
# 	push @CSVheader, "Benchmark Version";		## The benchmarkEngineVersion.  That is the fork and loop code that is being timed.
# 	push @CSVheader, "Beginning Uptime";		## uptime command
# 	push @CSVheader, "Ending Uptime";			## uptime command
# 	push @CSVheader, "Notes";					## User supplied notes from command line options.
# # 	push @CSVheader, "RSLTS TYPE";				## A uniquie id given to this running of the loops.
# 	push @CSVheader, "Innerloops";				## Number of iterations of the INNERLOOP.
# 
# 	for my $i ( 1 .. $maxPasses ) {
# 		push @CSVheader, "Pass #$i";
# 	}
# 
...
# 	my $CSVheader;
# 
# 	for my $i (0 .. $#CSVheader) {
# 		$CSVheader .= "$CSVheader[$i],";
# 	}
# 
# 
# 	$CSVheader =~ s/,$//;  ## Remove trailing comma.
# 
# 	return $CSVheader ;
# }
...
sub commandResults {
	...
	## Attempt to run a command with backticks.  If successful return result as a list.
	## Otherwise return 0 and print error to stdout.
	...
	## Get command given.  If none given then return.
	if ($_[0]) {
	...
	if ($?) { 
		...
		print STDERR "ERROR: Command failed with $?. [$command]\n";
		return 0;

	} else {

		chomp for @result;  # Remove newlines from result so as to be better for CSV.
...
sub winCommandResults {
	...
	if ($?) { 
		...
		return "Unknown. Command failed with $?.\n[$command]";
		...
		$result =~ s/\s+$//;   ## Remove trailing white space;
...
# sub sendCSV {
# 	
# 	## Given a list, output in CSV format.
# 	
# 	unless (defined $_[0]) { warn "Invalid input to sendCSV."; return; }
# 	
# 	my $arrayRef = $_[0];
# 
# 	my $lineOfCsv = "\n";  # Excel CSV does not terminate the last row with newline, so add newline to beginning of row output.
# 	
# 	
# 	foreach (@$arrayRef) {
# 	
...
#    			$field = "\"$_\"";  ## Enclose field in double quotes.
#  	
# 		} 
# 
# 		$lineOfCsv .= "$field,";
...
# 
# 
# 	## Substitute the last comma with nothing.
# 	$lineOfCsv =~ s/,$//;
# 	
# 
# 	return $lineOfCsv;	
...
sub sendCSV {
	...
	## Given a reference to a list, output referenced list as one row in CSV format.
	
	unless (defined $_[0] && ref $_[0] eq "ARRAY") { warn "Invalid input to sendCSV."; return "sendCSV failed at " . localtime(time) ; }
	
	my $lineOfCsv;
	
# 	my $arrayRef = $_[0];
	
	foreach (@{$_[0]}) {  ## First input paramater should be reference to array.
