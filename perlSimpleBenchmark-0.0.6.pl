...
## Start with 1 thread and increase by 1 thread each pass.
for my $i (1 .. $numberOfPasses) {
	#print "\nSleeping 3 seconds before starting benchmark pass.\n";  sleep 3;

	print "\nStarting pass $i of $numberOfPasses\n";
...
for my $path (@outDirs) {
	...
	unless ( -d $path ) { mkdir $path or die "Could not mkdir $path $!";}
...
#### Delta Times Row
push @deltaTimesRow, "Delta Times" ;

for my $i (1 .. $#passTimes) {

	#my ($threads, $time) = split / /,$passTimes[$ii],2;
	my $delta = $passTimes[$i][1];
# 	if ($i > 1) { $delta = $passTimes[$i][1]-$passTimes[$i-1][1]; }
	push @deltaTimesRow, $delta ;
}
...
# 	if ($i > 1) { $ips = (  ($numberofInnerLoops * $i) / $passTimes[$i][1])  / 1000000 ; }
	push @ipsRow, $ips  ;
}
...
## Send a copy of sysctl -a to a file

if ($sysctl_ref) {

	unless (-d "$outDir/sysctls") {
		mkdir "$outDir/sysctls" or warn "Could not make directory [$outDir/sysctls] $! ";
	}
	
	my $sysctlFile = "$outDir/sysctls/$uuid[0]-sysctl_-a.txt" ;
...
print "\n\n";
print "Check results written in [$outDir]\n";
...
sub perlSimpleBenchmarkEngine_0_3_0 {
	...
	#### The benchmark is the time it takes perl to complete an empty for loop
	...
	for my $ii (1 .. $threads) {
		...
		if ($pid == 0) {
			print "\tStarting thread $ii of $threads\n";
			my $innerStartTime= gettimeofday;

			for (1 .. $numberofInnerLoops ) {}    ## Empty for loop. 

			my $innerEndTime = gettimeofday;
			my $innerLoopElapsedTime = $innerEndTime - $innerStartTime;
			print "\t\tThread $ii time: $innerLoopElapsedTime\n";
			exit;		
		}
		push @pids, $pid;
...
sub parseArgs {
	...
	for my $i (0 .. $#ARGV) { 
		...
		## Extract -p which is number of passes
		if ($ARGV[$i] eq "-p") {
...
sub CSVheader {
	...
	push @header, "uname -a or ver";        ## uname -a command
	...
	push @header, "Results Type";           ## A uniquie id given to this running of the loops.

	for my $i ( 1 .. $maxnumberOfPasses ) {
		push @header, "p$i";
	}
